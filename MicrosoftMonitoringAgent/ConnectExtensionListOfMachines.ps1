...
Set-AzContext -SubscriptionId "$WsSubID"
...
foreach ($v in $csv)
{
    ...
    Set-AzContext -SubscriptionId "$v.SUBSCRIPTIONID"
    Remove-AzVMExtension -ResourceGroupName "$v.RSG" -Name "MicrosoftMonitoringAgent" -VMName "$v.OSNAME"
